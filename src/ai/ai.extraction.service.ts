...
export class AiExtractionService {
  ...
  private async callAIAndStore(
    ...
    interactionType: AIInteractionType,
    entityType: string,
  ): Promise<{ response: string; interaction: AIInteraction }> {
    let responseText = '';
    ...
    try {
      ...
      const interaction = await this.prismaService.aIInteraction.create({
        data: {
      ...
      return { response: responseText, interaction };
    } catch (error: any) {
      this.logger.error(`Error in ${interactionType}:`, error);
      ...
      throw new BadRequestException(
        `${interactionType} failed: ${error?.message}`,
      );
    }
  }
