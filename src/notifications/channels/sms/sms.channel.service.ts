...
@Injectable()
export class SmsChannelService implements OnModuleInit {
  ...
  onModuleInit() {
    if (!this.providers?.length) {
      this.logger.warn(
        'SMS channel is registered but has no active providers. ' +
          'Any SMS notification will be skipped. ' +
          'Enable TWILIO or AFRICASTALK in NotificationsModule.register() to activate SMS.',
      );
    }
  }
